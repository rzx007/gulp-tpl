...
var packPath = {
  ...
  jsLibs: ['./src/libs/**/*'], // 第三方库
  jsMain: ['./src/js/*.js', './src/views/*.js'],
  cssMian: ['./src/css/**/*.css', './src/views/**/*.css'],
  images: ['./src/assets/*'],
};
...
gulp.task('js_libs', function () {
  return gulp.src(packPath.jsLibs).pipe(gulp.dest('./dist/libs'));
});
// 压缩项目js
...
function watchs(path, task) {
  var watcher = gulp.watch(
    [].concat(packPath.html, packPath.jsMain, packPath.cssMian, packPath.jsLibs, packPath.images),
    gulp.series( 'html', 'css_main', 'images', 'js_libs', 'js_main')
  );
...
gulp.task(
  'build',
  gulp.series('production', 'clean', 'html', 'css_main', 'images', 'js_libs', 'js_main')
);

gulp.task(
  'dev',
  gulp.series('development', 'clean', 'html', 'css_main', 'images', 'js_libs', 'js_main', 'browser')
);
